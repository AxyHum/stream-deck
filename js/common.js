    ...
    function init() {
        ...
        function connect(args) {
            ...
            websocket.onopen = function() {
                ...
                console.log('Socket opened::connected:', {
                    connection: websocket,
                    port: inPort,
                    uuid: inUUID,
                    actionInfo: inActionInfo,
            ...
            websocket.onmessage = function(evt) {
                var jsonObj = Utils.parseJson(evt.data),
                    m;
                if (jsonObj.event && ['viewWillAppear'].includes(jsonObj.event)) {
                    //'sendToPlugin',
                    console.log('[STREAMDECK] websocket.onmessage', jsonObj.event, jsonObj);
                }
                // if (!['willAppear', 'willDisappear', 'titleParametersDidChange'].includes(jsonObj.event)) {
                //     console.log('[STREAMDECK] websocket.onmessage', jsonObj.event, jsonObj);
                // }

                if (!jsonObj.hasOwnProperty('action')) {
                    m = jsonObj.event;
                    // console.log('%c%s', 'color: white; background: grey; font-size: 12px;', '[common.js]onmessage:', m, jsonObj);
                } else {
                    ...
                    switch (inMessageType) {
                        ...
                        case 'registerPropertyInspector':
                            //   const e = jsonObj['event'];
                            //   if (filterPIMessages.includes(e)) {
                            //     console.log('EMIITING PIMESSAGE ', e, jsonObj);
                            //     events.emit(e, jsonObj);
                            //     m = jsonObj['action'] + '.' + jsonObj['event'];
                            //   } else {
                ...
                if (m && m !== '') {
                    // console.log('EMIITING ACTION MESSAGE ', m, jsonObj);
...
const SDApi = {
    ...
    common: {
        ...
        switchToProfile: function(inContext, inDeviceID, inProfileName) {
